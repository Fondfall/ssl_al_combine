...
def select(features, label, k=10, category=[0, 1, 2, 3]):
    """
    :param features: 输入样本的特征，x_i的特征为一维行向量
    :param label: 样本的标签，与features一一对应
    :param k: KNN的参数值
    ...
    for i in range(N):
        ...
        for j in range(N):
            ...
            if i == j:
                ...
                denominator = torch.dot(x_i, x_i) * torch.dot(x_j, x_j)
                denominator = round(denominator.item() ** 0.5, 2)
                # 若是单位向量，则不需要上面的运算
                similar[i, j] = torch.dot(x_i, x_j) / denominator
    # print(similar)
    '''
    step2 利用KNN生成伪标签
    '''
    ...
    for i in range(class_num):
        ...
        noise_label_index = noise_label_index + y_i_index[index_temp[select_num[i].long() + 1:]].long().tolist()
    ...
    print("sel_num:%d, others:%d" % (len(select_label_index), len(noise_label_index)))
    return select_label_features, select_label_index_order, noise_label_features, noise_label_index_order
